...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (args[0] === 'auto' || args[0] === '자동') {
            ...
            translate(args.slice(2).join(" "), { to: args[1] }).then(r => message.channel.send(new MessageEmbed().setAuthor('Google Translate', 'https://upload.wikimedia.org/wikipedia/commons/d/db/Google_Translate_Icon.png', 'https://translate.google.co.kr/').setTitle('구글 번역').setColor(0x00ff00).setDescription(`**${res.from.language.iso}** -> **${args[1]}**\n\n**번역 전**\n\`\`\`fix\n${args.slice(2).join(" ")}\n\`\`\`\n**번역 후**\n\`\`\`yml\n${r.text}\n\`\`\``))).catch(e => message.channel.send(`구글 번역기에서 지원하지 않은 언어이거나, 알 수 없는 오류가 발생했습니다\n${e}`))
        } else {
