...
module.exports = {
    ...
    run: async (client, message, args, ops) => {
        let res = await client.db.getAll().then(e => e.sort((a, b) => parseInt(b.value.money) - parseInt(a.value.money)).map((r, i) => `**${i+1}위** ${client.users.cache.get(r.key).tag} - ${parseInt(r.value.money).toLocaleString()}원`).join('\n'))
